...
/**
 *  @brief      Push Data_Gyro and Data_Accel orientation to the DMP.
 *  The orientation is represented here as the output of
 *  @e inv_orientation_matrix_to_scalar.
 *  @param[in]  orient  Gyro and Data_Accel orientation in body frame.
 *  @return     0 if successful.
 */
...
/**
 *  @brief      Push Data_Gyro biases to the DMP.
 *  Because the Data_Gyro integration is handled in the DMP, any Data_Gyro biases
 *  calculated by the MPL should be pushed down to DMP memory to remove
 *  3-axis quaternion drift.
 *  \n NOTE: If the DMP-based Data_Gyro calibration is enabled, the DMP will
...
/**
 *  @brief      Push Data_Accel biases to the DMP.
 ...
 *  @param[in]  axis    1, 2, and 4 for XYZ Data_Accel, respectively.
 ...
 *  If the DMP detects a Data_Gyro sample larger than @e thresh, taps are rejected.
 ...
 *  Sets the length of time that the Data_Gyro must be outside of the threshold set
 ...
 *  Sets the length of time after a shake rejection that the Data_Gyro must stay
...
int dmp_enable_feature(unsigned short mask)
{
    ...
    if (mask & (DMP_FEATURE_TAP | DMP_FEATURE_ANDROID_ORIENT)) tmp[0] = DINA20;
    else tmp[0] = 0xD8;
    mpu_write_mem(CFG_27, 1, tmp);

    if (mask & DMP_FEATURE_GYRO_CAL) dmp_enable_gyro_cal(1);
    else dmp_enable_gyro_cal(0);
    ...
    if (mask & DMP_FEATURE_LP_QUAT) dmp_enable_lp_quat(1);
    else dmp_enable_lp_quat(0);

    if (mask & DMP_FEATURE_6X_LP_QUAT) dmp_enable_6x_lp_quat(1);
    else dmp_enable_6x_lp_quat(0);
...
/**
 *  @brief      Calibrate the Data_Gyro data in the DMP.
 *  After eight seconds of no motion, the DMP will compute Data_Gyro biases and
 *  subtract them from the quaternion output. If @e dmp_enable_feature is
 *  called with @e DMP_FEATURE_SEND_CAL_GYRO, the biases will also be
 *  subtracted from the Data_Gyro output.
 *  @param[in]  enable  1 to enable Data_Gyro calibration.
 *  @return     0 if successful.
 */
 ...
 */
int dmp_read_fifo(
    short *Data_Gyro, short *Data_Accel, long *quat, unsigned long *timestamp, short *sensors, unsigned char *more)
{
    ...
    {
        Data_Accel[0] = ((short)fifo_data[ii + 0] << 8) | fifo_data[ii + 1];
        Data_Accel[1] = ((short)fifo_data[ii + 2] << 8) | fifo_data[ii + 3];
        Data_Accel[2] = ((short)fifo_data[ii + 4] << 8) | fifo_data[ii + 5];
        ii += 6;
    ...
    {
        Data_Gyro[0] = ((short)fifo_data[ii + 0] << 8) | fifo_data[ii + 1];
        Data_Gyro[1] = ((short)fifo_data[ii + 2] << 8) | fifo_data[ii + 3];
        Data_Gyro[2] = ((short)fifo_data[ii + 4] << 8) | fifo_data[ii + 5];
        ii += 6;
