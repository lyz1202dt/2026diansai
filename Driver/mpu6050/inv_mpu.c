...
#error Which Data_Gyro are you using? Define MPUxxxx in your compiler options.
#endif
...
struct chip_cfg_s
{
    ...
    /* 1 if device in low-power Data_Accel-only mode. */
...
/* Low-power Data_Accel wakeup rates. */
...
/**
 *  @brief      Enter low-power Data_Accel-only mode.
 *  In low-power Data_Accel mode, the chip goes to sleep and only wakes up to sample
...
int mpu_lp_accel_mode(unsigned short rate)
{
    ...
    /* For LP Data_Accel, we automatically configure the hardware to produce latched
     * interrupts. In LP Data_Accel mode, the hardware cycles into sleep mode before
...
#elif defined MPU6500
    /* Set wake frequency. */
    if (rate == 1) data = INV_LPA_0_98HZ;
    else if (rate == 2) data = INV_LPA_1_95HZ;
    else if (rate <= 5) data = INV_LPA_3_91HZ;
    else if (rate <= 10) data = INV_LPA_7_81HZ;
    else if (rate <= 20) data = INV_LPA_15_63HZ;
    else if (rate <= 40) data = INV_LPA_31_25HZ;
    else if (rate <= 70) data = INV_LPA_62_50HZ;
    else if (rate <= 125) data = INV_LPA_125HZ;
    else if (rate <= 250) data = INV_LPA_250HZ;
    else data = INV_LPA_500HZ;

    if (i2c_write(st.hw->addr, st.reg->lp_accel_odr, 1, &data)) return -1;

    if (i2c_read(st.hw->addr, st.reg->accel_cfg2, 1, &data)) return -1;
...
/**
 *  @brief      Read raw Data_Gyro data directly from the registers.
...
/**
 *  @brief      Read raw Data_Accel data directly from the registers.
 *  @param[out] data        Raw data in hardware units.
 *  @param[out] timestamp   Timestamp in milliseconds. Null if not needed.
 *  @return     0 if successful.
 */
...
/**
 *  @brief      Read biases to the Data_Accel bias 6500 registers.
 *  This function reads from the MPU6500 Data_Accel offset cancellations registers.
 *  The format are G in +-8G format. The register is initialized with OTP 
 *  factory trim values.
 *  @param[in]  accel_bias  returned structure with the Data_Accel bias
 *  @return     0 if successful.
 */
...
/**
 *  @brief      Read biases to the Data_Accel bias 6050 registers.
 *  This function reads from the MPU6050 Data_Accel offset cancellations registers.
 *  The format are G in +-8G format. The register is initialized with OTP 
 *  factory trim values.
 *  @param[in]  accel_bias  returned structure with the Data_Accel bias
 *  @return     0 if successful.
...
/**
 *  @brief      Push biases to the Data_Gyro bias 6500/6050 registers.
...
int mpu_set_gyro_bias_reg(long *gyro_bias)
{
    ...
    if (mpu_read_6500_gyro_bias(gyro_reg_bias)) return -1;
...
/**
 *  @brief      Push biases to the Data_Accel bias 6050 registers.
 *  This function expects biases relative to the current sensor output, and
 *  these biases will be added to the factory-supplied values. Bias inputs are LSB
 *  in +-8G format.
...
/**
 *  @brief      Push biases to the Data_Accel bias 6500 registers.
 *  This function expects biases relative to the current sensor output, and
 *  these biases will be added to the factory-supplied values. Bias inputs are LSB
 *  in +-8G format.
 *  @param[in]  accel_bias  New biases.
...
int mpu_reset_fifo(void)
{
    ...
    else
    {
        ...
        if (i2c_write(st.hw->addr, st.reg->int_enable, 1, &data)) return -1;
        if (i2c_write(st.hw->addr, st.reg->fifo_en, 1, &st.chip_cfg.fifo_enable)) return -1;
    }
    return 0;
}

/**
 *  @brief      Get the Data_Gyro full-scale range.
...
/**
 *  @brief      Set the Data_Gyro full-scale range.
...
/**
 *  @brief      Get the Data_Accel full-scale range.
 *  @param[out] fsr Current full-scale range.
 *  @return     0 if successful.
 */
...
/**
 *  @brief      Set the Data_Accel full-scale range.
 *  @param[in]  fsr Desired full-scale range.
 *  @return     0 if successful.
 */
...
#ifdef MPU6500 //MPU6500 Data_Accel/Data_Gyro dlpf separately
...
int mpu_set_sample_rate(unsigned short rate)
{
    ...
    else
    {
        if (st.chip_cfg.lp_accel_mode)
        {
            if (rate && (rate <= 40))
            {
                /* Just stay in low-power Data_Accel mode. */
                mpu_lp_accel_mode(rate);
                return 0;
            }
            /* Requested rate exceeds the allowed frequencies in LP Data_Accel mode,
 ...
 *  maximum of 100Hz. The actual rate can be set to a fraction of the Data_Gyro
 *  sampling rate.
 *
...
#ifdef AK89xx_SECONDARY
    unsigned char div;
    if (!rate || rate > st.chip_cfg.sample_rate || rate > MAX_COMPASS_SAMPLE_RATE) return -1;
...
/**
 *  @brief      Get Data_Gyro sensitivity scale factor.
...
/**
 *  @brief      Get Data_Accel sensitivity scale factor.
...
int mpu_set_sensors(unsigned char sensors)
{
    ...
    if (sensors && (sensors != INV_XYZ_ACCEL)) /* Latched interrupts only used in LP Data_Accel mode. */
        mpu_set_int_latched(0);

#ifdef AK89xx_SECONDARY
#ifdef AK89xx_BYPASS
    if (sensors & INV_XYZ_COMPASS) mpu_set_bypass(1);
    else mpu_set_bypass(0);
#else
    ...
    if (st.chip_cfg.dmp_on) user_ctrl |= BIT_DMP_EN;
    else user_ctrl &= ~BIT_DMP_EN;
 ...
 *  @param[out] Data_Gyro        Gyro data in hardware units.
 *  @param[out] Data_Accel       Accel data in hardware units.
 ...
 */
int mpu_read_fifo(
    short *Data_Gyro, short *Data_Accel, unsigned long *timestamp, unsigned char *sensors, unsigned char *more)
{
    /* Assumes maximum packet size is Data_Gyro (6) + Data_Accel (6). */
    ...
    {
        Data_Accel[0] = (data[index + 0] << 8) | data[index + 1];
        Data_Accel[1] = (data[index + 2] << 8) | data[index + 3];
        Data_Accel[2] = (data[index + 4] << 8) | data[index + 5];
        sensors[0] |= INV_XYZ_ACCEL;
        index += 6;
    }
    if ((index != packet_size) && st.chip_cfg.fifo_enable & INV_X_GYRO)
    {
        Data_Gyro[0] = (data[index + 0] << 8) | data[index + 1];
        sensors[0] |= INV_X_GYRO;
        index += 2;
    }
    if ((index != packet_size) && st.chip_cfg.fifo_enable & INV_Y_GYRO)
    {
        Data_Gyro[1] = (data[index + 0] << 8) | data[index + 1];
        sensors[0] |= INV_Y_GYRO;
        index += 2;
    }
    if ((index != packet_size) && st.chip_cfg.fifo_enable & INV_Z_GYRO)
    {
        Data_Gyro[2] = (data[index + 0] << 8) | data[index + 1];
...
static int compass_self_test(void)
{
    ...
    if (i2c_write(st.chip_cfg.compass_addr, AKM_REG_ASTC, 1, tmp)) goto AKM_restore;
    tmp[0] = AKM_MODE_SELF_TEST;
    if (i2c_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp)) goto AKM_restore;

    do
    {
        delay_ms(10);
        if (i2c_read(st.chip_cfg.compass_addr, AKM_REG_ST1, 1, tmp)) goto AKM_restore;
...
static int get_st_biases(long *Data_Gyro, long *Data_Accel, unsigned char hw_test)
{
    unsigned char data[MAX_PACKET_LENGTH];
    ...
    Data_Gyro[0] = Data_Gyro[1] = Data_Gyro[2] = 0;
    Data_Accel[0] = Data_Accel[1] = Data_Accel[2] = 0;
    ...
    for (ii = 0; ii < packet_count; ii++)
    {
        ...
        Data_Accel[0] += (long)accel_cur[0];
        Data_Accel[1] += (long)accel_cur[1];
        Data_Accel[2] += (long)accel_cur[2];
        gyro_cur[0] = (((short)data[6] << 8) | data[7]);
        gyro_cur[1] = (((short)data[8] << 8) | data[9]);
        gyro_cur[2] = (((short)data[10] << 8) | data[11]);
        Data_Gyro[0] += (long)gyro_cur[0];
        Data_Gyro[1] += (long)gyro_cur[1];
        Data_Gyro[2] += (long)gyro_cur[2];
    }
#ifdef EMPL_NO_64BIT
    Data_Gyro[0] = (long)(((float)Data_Gyro[0] * 65536.f) / test.gyro_sens / packet_count);
    Data_Gyro[1] = (long)(((float)Data_Gyro[1] * 65536.f) / test.gyro_sens / packet_count);
    Data_Gyro[2] = (long)(((float)Data_Gyro[2] * 65536.f) / test.gyro_sens / packet_count);
    if (has_accel)
    {
        Data_Accel[0] = (long)(((float)Data_Accel[0] * 65536.f) / test.accel_sens / packet_count);
        Data_Accel[1] = (long)(((float)Data_Accel[1] * 65536.f) / test.accel_sens / packet_count);
        Data_Accel[2] = (long)(((float)Data_Accel[2] * 65536.f) / test.accel_sens / packet_count);
        /* Don't remove gravity! */
        Data_Accel[2] -= 65536L;
    }
#else
    Data_Gyro[0] = (long)(((long long)Data_Gyro[0] << 16) / test.gyro_sens / packet_count);
    Data_Gyro[1] = (long)(((long long)Data_Gyro[1] << 16) / test.gyro_sens / packet_count);
    Data_Gyro[2] = (long)(((long long)Data_Gyro[2] << 16) / test.gyro_sens / packet_count);
    Data_Accel[0] = (long)(((long long)Data_Accel[0] << 16) / test.accel_sens / packet_count);
    Data_Accel[1] = (long)(((long long)Data_Accel[1] << 16) / test.accel_sens / packet_count);
    Data_Accel[2] = (long)(((long long)Data_Accel[2] << 16) / test.accel_sens / packet_count);
    /* Don't remove gravity! */
    if (Data_Accel[2] > 0L) Data_Accel[2] -= 65536L;
    else Data_Accel[2] += 65536L;
#endif
...
static int accel_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
    ...
    else
    {
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            {
                if (debug) log_i("Accel FAIL axis:%d <= 225mg or >= 675mg\n", i);
                result |= 1 << i; //Error condition
            }
        }
    }
    ...
    if (result == 0)
    {
        ...
        if (debug) log_i("Accel:CRITERIA C: bias less than %7.4f\n", accel_offset_max / 1.f);
        for (i = 0; i < 3; i++)
        {
            if (fabs(bias_regular[i]) > accel_offset_max)
            {
                if (debug) log_i("FAILED: Accel axis:%d = %d > 500mg\n", i, bias_regular[i]);
                result |= 1 << i; //Error condition
            }
        }
    }
...
static int gyro_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
    ...
    if (result == 0)
    {
        ...
        if (debug) log_i("Gyro:CRITERIA C: bias less than %7.4f\n", gyro_offset_max / 1.f);
        for (i = 0; i < 3; i++)
        {
            if (fabs(bias_regular[i]) > gyro_offset_max)
            {
                if (debug) log_i("FAILED: Gyro axis:%d = %d > 20dps\n", i, bias_regular[i]);
                result |= 1 << i; //Error condition
            }
        }
    }
...
static int get_st_6500_biases(long *Data_Gyro, long *Data_Accel, unsigned char hw_test, int debug)
{
    ...
    while (s < test.packet_thresh)
    {
        ...
        fifo_count = (data[0] << 8) | data[1];
        packet_count = fifo_count / MAX_PACKET_LENGTH;
        if ((test.packet_thresh - s) < packet_count) packet_count = test.packet_thresh - s;
    ...
    Data_Gyro[0] = (long)(((long long)Data_Gyro[0] << 16) / test.gyro_sens / s);
    Data_Gyro[1] = (long)(((long long)Data_Gyro[1] << 16) / test.gyro_sens / s);
    Data_Gyro[2] = (long)(((long long)Data_Gyro[2] << 16) / test.gyro_sens / s);
    Data_Accel[0] = (long)(((long long)Data_Accel[0] << 16) / test.accel_sens / s);
    Data_Accel[1] = (long)(((long long)Data_Accel[1] << 16) / test.accel_sens / s);
    Data_Accel[2] = (long)(((long long)Data_Accel[2] << 16) / test.accel_sens / s);
    ...
    if (debug)
    {
        log_i("Accel offset data HWST bit=%d: %7.4f %7.4f %7.4f\r\n",
              hw_test,
              Data_Accel[0] / 65536.f,
              Data_Accel[1] / 65536.f,
              Data_Accel[2] / 65536.f);
...
/**
 *  @brief      Trigger Data_Gyro/Data_Accel/compass self-test for MPU6500/MPU9250
 ...
 *  \n Bit 2:   Compass.
 *
 *  @param[out] Data_Gyro        Gyro biases in q16 format.
 *  @param[out] Data_Accel       Accel biases (if applicable) in q16 format.
 *  @param[in]  debug       Debug flag used to print out more detailed logs. Must first set up logging in Motion Driver.
 *  @return     Result mask (see above).
 */
int mpu_run_6500_self_test(long *Data_Gyro, long *Data_Accel, unsigned char debug)
{
    ...
    for (ii = 0; ii < tries; ii++)
        if (!get_st_6500_biases(Data_Gyro, Data_Accel, 0, debug)) break;
    if (ii == tries)
    {
    ...
    accel_result = accel_6500_self_test(Data_Accel, accel_st, debug);
    if (debug) log_i("Accel Self Test Results: %d\n", accel_result);

    gyro_result = gyro_6500_self_test(Data_Gyro, gyro_st, debug);
 ...
 *  @return     Result mask (see above).
 */
int mpu_run_self_test(long *Data_Gyro, long *Data_Accel)
{
#ifdef MPU6050
    const unsigned char tries = 2;
    long gyro_st[3], accel_st[3];
...
#if defined MPU6050
    for (ii = 0; ii < tries; ii++)
        if (!get_st_biases(Data_Gyro, Data_Accel, 0)) break;
    if (ii == tries)
    {
        /* If we reach this point, we most likely encountered an I2C error.
         * We'll just report an error for all three sensors.
    ...
    accel_result = accel_self_test(Data_Accel, accel_st);
    gyro_result = gyro_self_test(Data_Gyro, gyro_st);
...
restore:
     ...
     */
    get_st_biases(Data_Gyro, Data_Accel, 0);
    result = 0x7;
#endif
    /* Set to invalid values to ensure no I2C writes are skipped. */
    st.chip_cfg.gyro_fsr = 0xFF;
...
/**
 *  @brief      Enters LP Data_Accel motion interrupt mode.
 ...
 *  increments.
 *
 *  \n Low-power Data_Accel mode supports the following frequencies:
 *  \n 1.25Hz, 5Hz, 20Hz, 40Hz
 *
 *  \n MPU6500:
 *  \n Unlike the MPU6050 version, the hardware does not "lock in" a reference
 *  sample. The hardware monitors the Data_Accel data and detects any large change
 *  over a short period of time.
 *
 *  \n The hardware motion threshold can be between 4mg and 1020mg in 4mg
 *  increments.
 *
 *  \n MPU6500 Low-power Data_Accel mode supports the following frequencies:
 ...
 *  @param[in]  time        Duration in milliseconds that the Data_Accel data must
...
int mpu_lp_motion_interrupt(unsigned short thresh, unsigned char time, unsigned char lpa_freq)
{
    ...
    if (lpa_freq)
    {
        ...
        /* 1LSb = 4mg. */
        if (thresh > 1020) thresh_hw = 255;
        else if (thresh < 4) thresh_hw = 1;
        else thresh_hw = thresh >> 2;
#endif
        ...
        if (!st.chip_cfg.int_motion_only)
        {
            ...
            else st.chip_cfg.cache.dmp_on = 0;
        ...
        /* Enter full-power Data_Accel-only mode, no FIFO/DMP. */
        data[0] = 0;
        data[1] = 0;
        ...
        data[0] = thresh_hw;
        if (i2c_write(st.hw->addr, st.reg->motion_thr, 1, data)) goto lp_int_restore;

        /* Set wake frequency. */
        if (lpa_freq == 1) data[0] = INV_LPA_0_98HZ;
        else if (lpa_freq == 2) data[0] = INV_LPA_1_95HZ;
        else if (lpa_freq <= 5) data[0] = INV_LPA_3_91HZ;
        else if (lpa_freq <= 10) data[0] = INV_LPA_7_81HZ;
        else if (lpa_freq <= 20) data[0] = INV_LPA_15_63HZ;
        else if (lpa_freq <= 40) data[0] = INV_LPA_31_25HZ;
        else if (lpa_freq <= 70) data[0] = INV_LPA_62_50HZ;
        else if (lpa_freq <= 125) data[0] = INV_LPA_125HZ;
        else if (lpa_freq <= 250) data[0] = INV_LPA_250HZ;
        else data[0] = INV_LPA_500HZ;
        if (i2c_write(st.hw->addr, st.reg->lp_accel_odr, 1, data)) goto lp_int_restore;
        ...
        if (i2c_write(st.hw->addr, st.reg->accel_cfg2, 1, data)) goto lp_int_restore;

        /* Enable interrupt. */
        data[0] = BIT_MOT_INT_EN;
        if (i2c_write(st.hw->addr, st.reg->int_enable, 1, data)) goto lp_int_restore;

        /* Enable cycle mode. */
        data[0] = BIT_LPA_CYCLE;
        if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data)) goto lp_int_restore;
